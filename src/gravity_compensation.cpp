...
double force_dimension::Node::get_effector_mass() {
  double mass_kg = -1;
  int result =
      hardware_disabled_ ? 0 : dhdGetEffectorMass(&mass_kg, device_id_);
  if (result != 0) {
...
void force_dimension::Node::set_effector_mass(double mass_kg) {
  ...
  int result = hardware_disabled_ ? 0 : dhdSetEffectorMass(mass_kg, device_id_);
  if (result != 0) {
    std::string message = "Failed to get effector mass: ";
    message += hardware_disabled_ ? "unknown error" : dhdErrorGetLastStr();
    Log(message);
...
void force_dimension::Node::set_gravity_compensation(bool enable) {
  ...
  int result =
      hardware_disabled_ ? 0 : dhdSetGravityCompensation(val, device_id_);
  if (result != 0) {
    ...
    // Report the action.
    auto message = enable ? "Gravity compensation: Enabled"
...
void force_dimension::Node::set_gravity_compensation() {
  ...
  if (!success) {
    ...
    set_gravity_compensation(enable);
}
